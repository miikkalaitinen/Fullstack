...
blogRouter.put('/:id', middleware.tokenExtractor, async (request, response) => {
  ...
  if (
    blog.title.toString() === body.title.toString() &&
    blog.author.toString() === body.author.toString() &&
    body.url.toString() === body.url.toString()
  ) {
    ...
    await await Blog.findByIdAndUpdate(id, body, {
      new: true,
      runValidators: true,
      context: 'query',
    })
